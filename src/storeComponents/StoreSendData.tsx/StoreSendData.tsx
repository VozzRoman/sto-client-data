...
import { addTire } from '../../redux/operations';
...
//Owner
const [isOwner, setIsOwner] = useState<string>(selectOwner[0]);
...
setInputData(prev => {
	...
	const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		const response = await dispatch(addTire(data));
	...
	return (
		<form onSubmit={handleSubmit} className='text-white'>
				<div>
					...
					<h3 className='text-[20px] border-gray-400 mr-[10px] mb-[10px] text-orange-300 max-[840px]:text-[17px] whitespace-nowrap font-semibold'>Додати колеса</h3>
		 ...
		 <label className='block mr-5 max-[840px]:w-1/2'>
			...
			<label className='block mr-5 max-[840px]:mr-0 max-[840px]:w-1/2'>
				...
				pt-1 pb-1 pl-3 pr-3 rounded-md  transition-all duration-200 max-sm:w-full whitespace-nowrap`}>{isLoading ? "Завантажую" : "Додати до складу"}</button>
