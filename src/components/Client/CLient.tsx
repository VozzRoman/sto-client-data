...
const CLient:FC<ClientProps> = ({item, index, handelModalOpen, handleUpdateModalOpen, handelAlertWindowOpen}) => {
	...
	return (
		...
		<td className='relative' valign="middle">
			<Link onClick={() => handelModalOpen(item.id!)} className='underline hover:text-orange-600' to=''>{item.registrationNumber_1}</Link>
		...
		<td valign='middle' style={{textAlign:'center'}}>{item.tire_1.radius}</td>
