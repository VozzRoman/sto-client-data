...
const CLient:FC<ClientProps> = ({item, index, handelModalOpen, handleUpdateModalOpen, handelAlertWindowOpen}) => {
	...
	return (
		...
		<td className='relative h-[38px] border-b-[1px] max-sm:hidden visible' valign="middle">
			<Link onClick={() => handelModalOpen(item._id!)} className='underline hover:text-orange-600' to=''>{item.registrationNumber_1 ? item.registrationNumber_1 : 'відсутній'}</Link>
		...
		<Link onClick={() => handelModalOpen(item._id!)} className='underline hover:text-orange-600' to=''>
			{item.phone_1}
		</Link>
		...
		<button onClick={() => handleUpdateModalOpen(item._id!)} type="button" className="h-[32px] mr-[20px] max-md:mr-[10px] p-2 rounded-md text-white border-orange-400 bg-orange-400 hover:bg-orange-600 hover:border-orange-600"><FaEdit/></button>
		<button onClick={() => handelAlertWindowOpen(item._id!)} type="button" className="h-[32px] mr-[10px] max-md:mr-[2px] p-2 rounded-md text-white border-orange-400 bg-orange-400 hover:bg-orange-600 hover:border-orange-600"><MdDelete/></button>
		</td>	}	
		</tr>
